...
void Canvas::paintEvent(QPaintEvent *event)
{
    ...
    for(const cs1c::Shape* shape : vShapeList)
    {
        try
        {
            // Prone to segmentation fault if member
            // vars are incomplete
            shape->draw(device);
        } catch (std::exception e)
        {
            qDebug() << e.what() << "\nQPainter does not refer to Canvas.";
        }
    }
    ...
    //     LINE
    cs1c::Line myLine(&pPaint, 20, 90, 100, 20);
    ...
    //     Circle
    cs1c::Circle myCircle(&pPaint, 750, 150, 200);
    ...
    //     Rectangle
    cs1c::Rectangle myRect(&pPaint, 20, 200, 170, 100);
    ...
    //     Square
    cs1c::Square mySquare(&pPaint, 250, 150, 200);
    ...
    //     Ellipse
    cs1c::Ellipse myEllipse(&pPaint, 520, 200, 170, 100);
    ...
    //     Polyline
   cs1c::Polyline myPolyline(&pPaint);
